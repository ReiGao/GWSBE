...
while(<IN>){
	...
	if($tmp[2]==$tmp[3]){
		$flag=1 if($tmp[3]=~/A/ and $tmp[4]=~/G/);
		$flag=1 if($tmp[3]=~/T/ and $tmp[4]=~/C/);
	}elsif($tmp[2]==$tmp[4]){
        $flag=1 if($tmp[4]=~/A/ and $tmp[3]=~/G/);
        $flag=1 if($tmp[4]=~/T/ and $tmp[3]=~/C/);
	}else{
		print "ERRO $tmp[2]\n";
		next;
	}
...
close VCF;
close OUTLAST;
open(OUT,">ABE.jiaoji.summary.txt");
